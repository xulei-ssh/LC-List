...
namespace WordPlugin
{
  ...
  public class Formatting : IComparable
  {
    ...
    public Formatting()
    {
      _capsStyle = WordPlugin.CapsStyle.none;
      _strikethrough = WordPlugin.StrikeThrough.none;
      _script = WordPlugin.Script.none;
      _highlight = WordPlugin.Highlight.none;
      _underlineStyle = WordPlugin.UnderlineStyle.none;
      _misc = WordPlugin.Misc.none;
    ...
    internal XElement Xml
    {
      get
      {
        ...
        if( _underlineColor.HasValue )
        {
          ...
          if( _underlineStyle == WordPlugin.UnderlineStyle.none )
          {
            // Set the underlineStyle to the default
            _underlineStyle = WordPlugin.UnderlineStyle.singleLine;
            _rPr.Add( new XElement( XName.Get( "u", DocX.w.NamespaceName ), new XAttribute( XName.Get( "val", DocX.w.NamespaceName ), "single" ) ) );
          }
        ...
        if( _strikethrough.HasValue )
        {
          ...
          {
            case WordPlugin.StrikeThrough.none:
              break;
